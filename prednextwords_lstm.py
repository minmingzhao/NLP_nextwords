...
if not os.path.exists('datasets'):
    """
    firstly download the dataset and untar the file. 
    """
    os.mkdir('datasets')
    urllib.request.urlretrieve('http://www.fit.vutbr.cz/~imikolov/rnnlm/simple-examples.tgz','datasets/simple-examples.tgz')
    tar_ref = tarfile.open('datasets/simple-examples.tgz', 'r')
    tar_ref.extractall('datasets')
    tar_ref.close()

data_path = "datasets/simple-examples/data"
mode = 'train'
...
def build_vocab(filename):
    ...
    count_pairs = sorted(counter.items(), key=lambda x: (-x[1], x[0]))  # swap (word, count) as (count, word)
...
if __name__ == '__main__':
    ...
    print(model.summary())
    checkpointer = ModelCheckpoint(filepath='model/model.hdf5', verbose=1)
    tensorboard = TensorBoard(log_dir='./logs', histogram_freq=1, write_graph=True, write_grads=True, write_images=True, batch_size=32)
    num_epochs = 50
    if args.mode == 'train':
        if os.path.exists('model/model.hdf5'):
            model = load_model('model/model.hdf5')
            print('!!!prev trained model weights reloaded!!!')
